    ...
    for j in range(len(fecha_informe)):
        try:            
            if year_informe[i] == 2019: 
                ...
                driver.find_element_by_css_selector('#recuadrosxAno_group_pvid_' + str(id_informe[i]) + ' > div:nth-child(' + str(path_id_a[j] - 1) + ') > div:nth-child(' + str(path_id_b[j]) + ') > div.media-body > h3 > a').click()
...
np.save('tablas_apf.npy', tables_finale)    
tables = np.load('tablas_apf.npy', allow_pickle=True)    
...
indicadores_afp = pd.concat([pd.DataFrame(total_activos_A), pd.DataFrame(total_activos_E), 
                             pd.DataFrame(renta_variable_A), pd.DataFrame(renta_variable_E)], axis = 1)
    
indicadores_afp.columns = ['renta A', 'renta E', 'int var A', 'int var E']
    
for i in range(4):
    indicadores_afp.iloc[:,i] = indicadores_afp.iloc[:,i].str.replace('.','').str.replace(',','.').astype(float)
    
locale.setlocale(locale.LC_TIME, 'es_ES.UTF-8')
dates = pd.date_range('2010-01-01','2019-10-01', freq='MS').strftime("%Y-%m-%d %H:%M:%S")
...
indicadores_afp.to_excel('indicadores_afp.xlsx')
