...
id_informe = [10089]
fecha_informe = ['enero', 'febrero', "marzo"]
path_id_b = [2, 1, 1] # ultimo 1 por estar solo un mes
path_id_a = [1, 2, 1] # ultimo 1 por estar solo un mes
year_informe = [2020]
page_table = [15, 15, 15]
tables_finale = []
...
for i in range(len(id_informe)):
    ...
    for j in range(len(fecha_informe)):
        try:            
            ...
            driver.find_element_by_css_selector('#recuadrosxAno_group_pvid_' + str(10274) + '> div:nth-child(' + str(path_id_a[j]) + ') > div:nth-child(' + str(path_id_b[j]) + ') > div.media-body > h3 > a').click()
...
# np.save('tablas_apf_from_2020.npy', tables_finale)    
...
for i in range(len(tables_finale)):
    afp_rendimiento = pd.DataFrame(tables_finale[i])
...
dates = pd.date_range('2020-01-01','2020-03-01', freq='MS').strftime("%Y-%m-%d %H:%M:%S")
...
dolar_obs_monthly = dolar_obs_monthly[dolar_obs_monthly.index >= '2020-01-01 00:00:00']
...
indicadores_afp.to_excel('indicadores_afp_from_2020.xlsx')

#%%
afp_until_2019 = pd.read_excel("indicadores_afp_until_2019.xlsx", index_col = 0)
afp_from_2020 = pd.read_excel("indicadores_afp_from_2020.xlsx", index_col = 0)

afp_updated = pd.concat([afp_until_2019, afp_from_2020], axis = 0)

afp_updated.to_excel('indicadores_afp_updated.xlsx')
